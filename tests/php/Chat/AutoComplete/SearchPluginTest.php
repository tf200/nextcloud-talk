...
class SearchPluginTest extends TestCase {
	...
	protected OrganizationIntegrationService&MockObject $organizationIntegrationService;
	...
	public function setUp(): void {
		...
		$this->organizationIntegrationService = $this->createMock(OrganizationIntegrationService::class);
	...
	protected function getPlugin(array $methods = []) {
		if (empty($methods)) {
			return new SearchPlugin(
				...
				$this->organizationIntegrationService,
			);
		}
	...
	public function testSearch(): void {
		...
		$this->organizationIntegrationService->expects($this->once())
			->method('canUserUseTalk')
			->with('current')
			->willReturn(true);
		$this->organizationIntegrationService->expects($this->once())
			->method('filterReachableUserIds')
			->with('current', ['123', 'foo', 'bar'])
			->willReturn(['123', 'foo', 'bar']);
		$this->organizationIntegrationService->expects($this->exactly(2))
	...
	public function testSearchReturnsNoResultsWhenTalkIsBlocked(): void {
		$result = $this->createMock(ISearchResult::class);
		$room = $this->createMock(Room::class);
		...
		$this->assertFalse($plugin->search('fo', 10, 0, $result));
	}
