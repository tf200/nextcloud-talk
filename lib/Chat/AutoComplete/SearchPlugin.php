...
class SearchPlugin implements ISearchPlugin {
	...
	public function search($search, $limit, $offset, ISearchResult $searchResult): bool {
		...
		if ($this->room->getObjectType() === 'file') {
			...
			if (!empty($usersWithFileAccess)) {
				...
				$this->searchUsers($search, $this->filterAllowedUsers($users), $searchResult);
			}
		}
		...
		$userIds = $this->filterAllowedUsers($userIds);
	 ...
	 */
	private function filterAllowedUsers(array $userIds): array {
		if ($this->organizationIntegrationService->isGlobalAdmin($this->userId)) {
			return $userIds;
		}

		$allowedUserIds = array_flip($this->organizationIntegrationService->filterReachableUserIds($this->userId, array_keys($userIds)));

		return array_filter(
			$userIds,
			static fn (string $displayName, string $userId): bool => isset($allowedUserIds[$userId]),
