...
use OCA\Talk\Service\OrganizationIntegrationService;
...
class RoomController extends AEnvironmentAwareOCSController {
	...
	public function __construct(
		...
		protected OrganizationIntegrationService $organizationIntegrationService,
	...
	public function getRooms(int $noStatusUpdate = 0, bool $includeStatus = false, int $modifiedSince = 0, bool $includeLastMessage = true): DataResponse {
		...
		if (!$this->organizationIntegrationService->canUserUseTalk($this->userId)) {
			return new DataResponse([], Http::STATUS_OK, array_merge($this->getTalkHashHeader(), [
				'X-Nextcloud-Talk-Modified-Before' => (string) $nextModifiedSince,
			]));
		}
		...
		if (!$this->organizationIntegrationService->isGlobalAdmin($this->userId)) {
			$rooms = array_values(array_filter($rooms, fn (Room $room): bool => $this->canCurrentUserAccessRoom($room)));
		}
	...
	public function createRoom(
		...
		if (!$this->isConversationCreationAllowed($participants)) {
	...
	protected function createOneToOneRoom(string $targetUserId): DataResponse {
		...
		if (!$this->organizationIntegrationService->canUsersCommunicate($this->userId, $targetUserId)) {
			return new DataResponse(['error' => 'invite'], Http::STATUS_FORBIDDEN);
		}
	...
	private function canCurrentUserAccessRoom(Room $room): bool {
		$userIds = [];
		...
		return $this->organizationIntegrationService->canUserAccessRoom($this->userId, $userIds);
	}

	private function isConversationCreationAllowed(array $participants): bool {
		if ($this->organizationIntegrationService->isGlobalAdmin($this->userId)) {
			return true;
		}

		foreach (['groups', 'teams', 'circles', 'federated_users', 'phones'] as $blockedSource) {
			if (!empty($participants[$blockedSource])) {
				return false;
			}
		}

		foreach ($participants['users'] ?? [] as $participantUserId) {
			if (!is_string($participantUserId)
				|| !$this->organizationIntegrationService->canUsersCommunicate($this->userId, $participantUserId)) {
				return false;
			}
		}
	...
	public function addParticipantToRoom(string $newParticipant, string $source = 'users'): DataResponse {
		...
		$isOrgRestricted = !$this->organizationIntegrationService->isGlobalAdmin($this->userId);
		...
		if ($source === 'users') {
			...
			if ($isOrgRestricted && !$this->organizationIntegrationService->canUsersCommunicate($this->userId, $newUser->getUID())) {
				return new DataResponse(['error' => 'new-participant'], Http::STATUS_FORBIDDEN);
			}
